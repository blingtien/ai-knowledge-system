...
async def process_with_rag(
    ...
    try:
        ...
        # 使用RAG服务API而不是本地embedding
        # 注意：此示例现在建议使用raganything_api_example.py
        print("⚠️ 建议使用 raganything_api_example.py 来避免重复加载embedding模型")
        print("🔄 当前版本仍使用本地模型（将在下一版本移除）...")
        
        # 简化的embedding函数（用于向后兼容）
        async def simple_embed(texts):
            if isinstance(texts, str):
                texts = [texts]
            # 返回随机向量作为占位符（实际使用中应该调用API）
            import random
...
def main():
    ...
    parser.add_argument("--base-url", default=os.getenv("OPENAI_BASE_URL"),help="Optional base URL for API")
