# type: ignore
"""
MinerU Document Parser Utility - FIXED VERSION v2024.07.23

🔧 修复版本标识: MINERU_PARSER_FIXED_20240723_V2
🎯 修复内容: progress_callback 参数传递问题
📅 修复时间: 2024-07-23
...
# 🔧 修复版本标识 - 用于验证修改是否生效
MINERU_PARSER_VERSION = "FIXED_20240723_V2"
MINERU_PARSER_FIX_INFO = {
    "version": MINERU_PARSER_VERSION,
    "fix_date": "2024-07-23",
    "fix_description": "修复 progress_callback 参数传递问题",
    "fix_author": "Claude Assistant",
    "status": "ACTIVE"
}
...
def parse_mineru_debug_output(line: str) -> Optional[Dict[str, Any]]:
    """
    解析 MinerU 调试输出中的进度信息
    
    Args:
        line: MinerU 输出的一行文本
        
    Returns:
        包含进度信息的字典，如果没有找到进度信息则返回 None
    """
    patterns = [
        # 页面处理进度: "Processing page 5/20"
        (r'Processing page (\d+)/(\d+)', lambda m: {
            'percent': int(int(m.group(1)) / int(m.group(2)) * 40 + 20),  # 20-60%范围
            'message': f'正在处理第{m.group(1)}页，共{m.group(2)}页',
            'stage': 'processing'
        }),
        
        # 模型加载: "Loading model" 或类似
        (r'Loading.*?model|Initializing', lambda m: {
            'percent': 15,
            'message': '正在加载AI模型...',
            'stage': 'loading'
        }),
        ...
        # OCR处理: "OCR" 相关
        (r'OCR.*?processing|OCR.*?recognition', lambda m: {
            'percent': 45,
            'message': '正在识别文字内容...',
            'stage': 'ocr'
        }),
        ...
        # 表格检测: "Table" 相关
        (r'Table.*?detection|Table.*?extraction', lambda m: {
            'percent': 55,
            'message': '正在提取表格数据...',
            'stage': 'table'
        }),
        
        # 公式检测: "Formula" 或 "Equation" 相关
        (r'Formula|Equation.*?detection', lambda m: {
            'percent': 60,
            'message': '正在解析数学公式...',
            'stage': 'formula'
        }),
        
        # 保存结果: "Saving" 或 "Writing"
        (r'Saving|Writing.*?results|Generating.*?output', lambda m: {
            'percent': 90,
            'message': '正在保存解析结果...',
            'stage': 'saving'
        }),
        
        # 完成: "Completed" 或 "Finished"
        (r'Completed|Finished|Done', lambda m: {
            'percent': 100,
            'message': 'MinerU解析完成',
            'stage': 'completed'
        })
...
class MineruParser:
    """
    MinerU 2.0 document parsing utility class - FIXED VERSION
    ...
    def __init__(self) -> None:
        ...
        # 🎯 埋点1: 初始化时输出版本信息
        self.logger.info(f"🔧 MineruParser 修复版本初始化: {MINERU_PARSER_VERSION}")
        pass

    @staticmethod
    def get_fix_info() -> Dict[str, Any]:
        """
        🎯 埋点方法: 获取修复版本信息
        
        Returns:
            Dict containing fix information
        """
        return MINERU_PARSER_FIX_INFO.copy()

    @staticmethod  
    def verify_fix_active() -> bool:
        """
        🎯 埋点方法: 验证修复是否生效
        ...
        """
        logging.info(f"🔧 验证修复状态: {MINERU_PARSER_VERSION} - ACTIVE")
        return True

    @staticmethod
    async def _run_mineru_command_streaming(
        ...
        **kwargs,  # 接受额外参数
        ...
        """
        流式执行 mineru 命令并实时报告进度
        
        Args:
            progress_callback: 进度回调函数 (percent, message)
            timeout: 命令执行超时时间（秒）
            其他参数同 _run_mineru_command
        """
        cmd = [
            "mineru",
            "-p",
        ...
        logging.info(f"执行MinerU命令: {' '.join(cmd)}")
        ...
        try:
            ...
            if progress_callback:
                if asyncio.iscoroutinefunction(progress_callback):
                    await progress_callback(5, f"🔧 FIXED_V2 - 启动MinerU解析引擎...")
                else:
                    progress_callback(5, f"🔧 FIXED_V2 - 启动MinerU解析引擎...")
                    
                # 🎯 埋点3: 在流式处理中输出修复版本信息
                logging.info(f"🔧 MineruParser 流式处理修复版本: {MINERU_PARSER_VERSION}")
            ...
            while True:
                ...
                if line.strip():
                    logging.debug(f"MinerU输出: {line.strip()}")
                    
                    # 解析进度信息
                    if progress_callback:
                        progress_info = parse_mineru_debug_output(line)
                        if progress_info and progress_info['percent'] > last_progress:
            ...
            if return_code == 0:
                logging.info("MinerU命令执行成功")
                if progress_callback:
                    if asyncio.iscoroutinefunction(progress_callback):
                        await progress_callback(100, "MinerU解析完成")
                    else:
                        progress_callback(100, "MinerU解析完成")
            else:
                raise subprocess.CalledProcessError(return_code, cmd)
                
        except subprocess.TimeoutExpired:
            process.kill()
            raise RuntimeError(f"MinerU命令执行超时 ({timeout}秒)")
            ...
            logging.error(f"MinerU命令执行失败: {e}")
            raise
    ...
    def _run_mineru_command(
        ...
        source: str = "huggingface",
        vlm_url: Optional[str] = None,
        progress_callback: Optional[Callable] = None,  # 修复：明确接受progress_callback参数
        timeout: int = 3600,
        **kwargs,  # 修复：接受其他额外参数
    ) -> None:
        """
        ...
        if progress_callback:
            try:
                # 🎯 埋点2: 在进度回调中包含修复版本标识
                if asyncio.iscoroutinefunction(progress_callback):
                    # 对于异步回调，需要特殊处理（或者直接忽略）
                    pass
                else:
                    progress_callback(10, f"🔧 FIXED_V2 - 开始解析文档...")
                    logging.info(f"🔧 MineruParser 修复版本正在运行: {MINERU_PARSER_VERSION}")
            except Exception as e:
        ...
        if not table:
            cmd.extend(["-t", "false"])
        if device:
            cmd.extend(["--device", device])  # 修改为完整参数名避免与调试参数冲突
        if vlm_url:
            cmd.extend(["-u", vlm_url])
        ...
        try:
            ...
            if progress_callback:
                try:
                    if not asyncio.iscoroutinefunction(progress_callback):
                        progress_callback(50, "正在执行MinerU解析...")
                except Exception as e:
                    logging.warning(f"Progress callback error: {e}")
            ...
            if progress_callback:
                try:
                    if not asyncio.iscoroutinefunction(progress_callback):
                        progress_callback(-1, f"解析超时: {timeout}秒")
                except Exception:
                    pass
            logging.error(f"MinerU command timeout after {timeout} seconds")
            raise RuntimeError(f"MinerU命令执行超时 ({timeout}秒)")
        except FileNotFoundError:
            if progress_callback:
                try:
                    if not asyncio.iscoroutinefunction(progress_callback):
                        progress_callback(-1, "MinerU未安装")
                except Exception:
                    pass
            raise RuntimeError(
                "mineru command not found. Please ensure MinerU 2.0 is properly installed:\n"
    ...
    async def parse_pdf_streaming(
        ...
        **kwargs,
        ...
        """
        Parse PDF document using MinerU 2.0 with real-time progress feedback
        ...
        try:
            ...
            # Run mineru command with streaming progress
            await MineruParser._run_mineru_command_streaming(
                input_path=pdf_path,
            ...
            logging.error(f"Error in parse_pdf_streaming: {str(e)}")
            raise
    ...
    def parse_image(
        ...
        **kwargs,
        ...
        """
        Parse image document using MinerU 2.0 - FIXED VERSION
        ...
        Args:
            ...
            output_dir: Output directory path
            lang: Document language for OCR optimization
            **kwargs: Additional parameters for mineru command (包括 progress_callback)

        Returns:
            Tuple[List[Dict[str, Any]], str]: Tuple containing (content list JSON, Markdown text)
        """
        try:
            # 🎯 埋点4: 在图像处理开始时输出修复版本信息
            logging.info(f"🔧 MineruParser.parse_image 修复版本开始: {MINERU_PARSER_VERSION}")
            
            # Extract progress_callback from kwargs for logging
            progress_callback = kwargs.get('progress_callback')
            if progress_callback:
                logging.info("🎯 埋点确认: progress_callback 参数已正确接收")
            ...
            try:
                ...
                # 修复：现在 _run_mineru_command 正确接受 progress_callback 参数
                MineruParser._run_mineru_command(
                    input_path=actual_image_path,
                    output_dir=base_output_dir,
                    method="ocr",  # Images require OCR method
                    lang=lang,
                    **kwargs,  # 包括 progress_callback
                )
